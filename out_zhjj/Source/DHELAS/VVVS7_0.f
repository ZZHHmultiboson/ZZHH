...
C     
      SUBROUTINE VVVS7_0(V1, V2, V3, S4, COUP,VERTEX)
      IMPLICIT NONE
      COMPLEX*16 CI
      ...
      VERTEX = COUP*S4(3)*(TMP13*(TMP1*(-1D0)*(+CI*(TMP14+TMP15))
     $ +(TMP4*(-CI*(TMP23)+CI*(TMP12))+TMP7*(+CI*(TMP15+TMP10))))
     $ +(TMP16*(TMP14*(-CI*(TMP8)+CI*(TMP5))+(TMP2*(+CI*(TMP23)-CI
     $ *(TMP11))+(-CI*(TMP8*TMP10)+CI*(TMP5*TMP15))))+(TMP9*(TMP0*(
     $ -1D0)*(+CI*(TMP15+TMP10))+(TMP3*(+CI*(TMP14+TMP10))+TMP6*(+CI
     $ *(TMP11)-CI*(TMP12))))+(TMP0*(+CI*(TMP1*TMP2+TMP4*TMP8))+(TMP3
     $ *(-1D0)*(+CI*(TMP2*TMP7+TMP4*TMP5))+TMP6*(-CI*(TMP5*TMP7)+CI
     $ *(TMP1*TMP8)))))))
      END
